...
def test_structured_response_selects_one_schema_valid_candidate():
    ...
    noisy_document = document.model_dump()
    noisy_document['blocks'][0].pop('stage')
    parsed = WriterToolBase()._validate_structured_response(
        [{'level': 1}, noisy_document, noisy_document],
        WriterDocument,
    )
    assert parsed == document


def test_text_response_strips_provider_think_block():
...
def test_stream_markdown_draft_is_isolated_and_returns_tool_result():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        with tool.stream_draft_section(
            ...
            media_assets=MediaAssetLibrary(library_id='media-library-stream'),
            idle_timeout=1,
        ) as stream:
...
def test_markdown_draft_receives_only_its_resolved_media():
    task, instruction, context = _markdown_draft_inputs()
    plan = VisualPlan(instructions=[VisualInstruction(
        need_id='IMAGE-1',
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        with patch.object(
            ...
            result = tool.generate_draft_section(
                task, instruction, context, visual_plan=plan, media_assets=library,
            )
