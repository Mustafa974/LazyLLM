...
@pytest.mark.parametrize(
    ('visual_type', 'preferred_strategy'),
    [
        ('image', 'web_search'),
        ('diagram', 'code_render'),
        ('chart', 'web_search'),
...
def test_normalize_revision_visual_plan_supports_all_visual_types(
    visual_type,
    preferred_strategy,
):
    content_ref = ContentRef(node_id='anchor')
    plan = ModifyPlan(
        scope='block',
        instructions=[ModifyInstruction(
            instruction_id=f'{visual_type}-1',
            content_ref=content_ref,
            modify_type='create',
            position='after',
            instruction='新增视觉内容',
    ...
    normalized = WriterRevisionTools()._normalize_modify_plan(
        plan,
        WritingTask(query='新增视觉内容', task_type='revise'),
        [content_ref],
        {WriterRevisionTools._content_ref_key(content_ref)},
    )

    assert normalized.instructions[0].visual_instruction.visual_type == visual_type
    assert normalized.instructions[0].visual_instruction.preferred_strategy == preferred_strategy


def test_normalize_revision_visual_plan_rejects_wrong_strategy_for_type():
    content_ref = ContentRef(node_id='anchor')
    plan = ModifyPlan(
        scope='block',
        instructions=[ModifyInstruction(
            ...
            visual_instruction=VisualInstruction(
                ...
                visual_type='image',
                purpose='说明正文中的关键内容',
                preferred_strategy='code_render',
            ),
        )],
    )

    with pytest.raises(ValueError, match='not supported by visual_type'):
        WriterRevisionTools()._normalize_modify_plan(
            plan,
            WritingTask(query='新增图片', task_type='revise'),
            [content_ref],
            {WriterRevisionTools._content_ref_key(content_ref)},
        )
