...
from ..data_models.multimodal import _VISUAL_STRATEGY_ORDER, VisualPlan
...
class WriterPlanningTools(WriterToolBase):
    ...
    def generate_outline(
        ...
        if resolved_representation == 'markdown':
            outline = self._call_llm_text(
                self._markdown_outline_prompt(
                    writing_task, writing_context, profiles, execution_data,
                )
            )
            return self._save_markdown_outline(
                outline, writing_task, writing_context, profiles, execution_data,
            )

        document_id = f'{writing_context.context_id}-outline'
        outline = self._call_llm_structured(
            self._ir_outline_prompt(
                writing_task, writing_context, profiles, execution_data, document_id,
            ),
            WriterDocument,
        )
        outline = self._normalize_streamed_outline(
            outline, document_id, writing_task, writing_context, profiles,
        )
        return self._save_ir_outline(
            outline, writing_task, writing_context, profiles, execution_data,
        )
    ...
    def stream_outline(
        ...
        *,
        ...
        if resolved_representation == 'markdown':
            prompt = self._markdown_outline_prompt(
                writing_task, writing_context, profiles, execution_data,
            )
        ...
        document_id = f'{writing_context.context_id}-outline'
        prompt = self._ir_outline_prompt(
            writing_task, writing_context, profiles, execution_data, document_id,
        )
        preview_title = (
    ...
    @staticmethod
    def _markdown_outline_prompt(
        task: WritingTask,
        context: WritingContext,
        profiles: List[ResourceProfile],
        execution_data: Any,
    ) -> str:
        return GENERATE_OUTLINE_MARKDOWN_PROMPT.format(
