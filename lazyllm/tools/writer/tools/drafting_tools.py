...
class WriterDraftingTools(WriterToolBase):
    ...
    def _markdown_draft_prompt(
        ...
        media_library = self._unified_optional_model(media_assets, MediaAssetLibrary)
        section_media = self._markdown_media_for_section(instruction, resolved_visual_plan, media_library)
    ...
    def _markdown_media_for_section(
        ...
        needs = [need for need in visual_plan.instructions
                 if (tuple(need.content_ref.heading_path), need.content_ref.occurrence) == key]
        library = library or MediaAssetLibrary(library_id='')
        asset_ids = {asset_id for need in needs
                     for asset_id in library.visual_need_asset_ids.get(need.need_id, [])}
        return {
            'visual_needs': [
                {'need_id': need.need_id, 'visual_type': need.visual_type,
                 'purpose': need.purpose, 'required': need.required} for need in needs
            ],
            'assets': [
                {'media_asset_id': asset.media_asset_id, 'asset_type': asset.asset_type,
                 'caption': asset.caption, 'summary': asset.summary}
                for asset_id, asset in library.assets.items() if asset_id in asset_ids
            ],
            'visual_need_asset_ids': {
                need.need_id: library.visual_need_asset_ids.get(need.need_id, []) for need in needs
            },
        }
